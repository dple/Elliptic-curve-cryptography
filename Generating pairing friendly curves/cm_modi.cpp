...
BOOL get_curve(Complex& lam,Float *Fi2,ofstream& ofile,ofstream& ofile_defaut,Big r,Big other_r,Big ord,unsigned long D,Big p,Big n,Big W,int m,BOOL always)
{
    Poly polly;
    Big A0,B0,k;
    BOOL unstable;
    ...
    {
    	cout << "class number= " << class_number << endl;
	cout << "Trace Frobenius = " << n - p - 1 << endl;
    	cout << "Group order= " << ord << endl;
    }
    else
    {	
...
// A.12.1
	...
	//ofile_defaut << endl << "Elliptic curve E is defined over the finite field Fp !" << endl;
	//ofile_defaut << "The number of bits of the characteristic of finite field p = " << bits(p) << endl;
        //mip->IOBASE=16;
        //ofile_defaut << "p = " << p << endl;
	//ofile_defaut << "The output curve is of the form y^2 = x^3 + ax + b, where:" << endl;
        ofile_defaut << "A = " << a << endl;
        ofile_defaut << "B = " << b << endl;
        //ofile_defaut << "The order of E = " << ord << endl << endl;
	//ofile_defaut << "Coordinates of the random point P :" << ord << endl; 
	ofstream tmp;
	tmp.open("tmp.txt");
	tmp << a << endl;
	tmp << b << endl;
	tmp.close();
        ...
        if (pord)
        {
            ofile_defaut << "x = " << x << endl;
            ofile_defaut << "y = " << y << endl;
        }	    
        //mip->IOBASE=10;
	}
    exit(0);
...
//	cout << "Incorrect Usage" << endl;
    cout << "Program tries to find Elliptic Curve mod a prime P, with its order N and point of prime order" << endl;
	...
	cout << "cm_modi <prime number P> -n <big number N>" << endl;
	cout << "OR" << endl;
	cout << "cm_modi -f <formula for P> -n <big number N>" << endl;
#if defined(unix)
...
int choose_k(void) {
	...
	fgets(szInput, 2, stdin);
	i = atoi(szInput);
  return i;
}

void get_parameters(Big &n, Big &p, unsigned long &SD, unsigned int &sk) {
	char c_curve, c_secu;
	unsigned int k;
...
int main(int argc,char **argv)
{
    ...
    unsigned long SD,D;
    unsigned int sk;
    ofstream ofile, ofile_defaut;
    ifstream ifile ("curve_para.txt");
    ...
    irand(0L);
	sk = 0;
	ofile_defaut.open("curve_output.txt", ios::out | ios::app);
	if (!ofile_defaut.is_open()) {
		cout << "Can't open the file [curve_output.txt] to write !" << endl;
		exit(0);
	}
...
/*    if (argc < 1)
    {
     cout << "Incorrect Usage" << endl;
      ...
      cout << "that is a point (X,Y) on the curve y^2=x^3+Ax+B of order R" << endl;
      cout << "where R is large and prime > |P/K|. (K defaults to 256)" << endl;
      cout << "cm <prime number P> -n <big number N>" << endl;
      cout << "OR" << endl;
      cout << "cm -f <formula for P> -n <big number N>" << endl;
#if defined(unix)
      cout << "e.g. cm -f 2^192-2^64-1" << endl;
#else
      cout << "e.g. cm -f 2#192-2#64-1" << endl;
...
/*		sk = atoi(kin);
		...
		if (ifile.is_open()) {
			...
			sk = atoi(line); //cout << "The embedding degree k = " << sk << endl;
			//p = line; cout << "Big p =" << p << endl;
			ifile.getline(line, 256); 
			SD = atol(line); //cout << "The discriminant D = " << SD << endl;
			ifile.getline(line, 256);
			p = line; 
			cout << "p = " << line << endl;
			ifile.getline(line, 256);
		...
		//get_parameters(n, p, SD, sk);
	}

    
    if (n==0)
    {
	cout << "Error in command line. Usage ./cm <Prime number P> -n <Big number N>" << endl;
        return 0;
    }
...
// A.14.2.3
        ...
        if (ord>=rmin && (k==0 || prime(ord))) 
            get_curve(lam,Fi2,ofile,ofile_defaut,r1,r2,ord,D,p,n,W,m,always);
        ...
        if (ord>=rmin && (k==0 || prime(ord))) 
            get_curve(lam,Fi2,ofile,ofile_defaut,r2,r1,ord,D,p,n,W,m,always);
