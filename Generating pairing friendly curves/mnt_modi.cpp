...
/*    if (argc<1)
    ...
    pFile.open("curve_para.txt", ios::in | ios::trunc);
    if (!pFile.is_open()) {
	cout << "Can't open file [curve_para.txt] to write !\n";
	exit(0);
    }
    if (argc == 0) {
		...
		cout << "Can't open the parameter file \n";
		exit(0);
	}
	...
	// N=atoi(argv[0]);
    if (N!=3 && N!=4 && N!=6)   {
       	cout << "Wrong parameter - for MNT curve the embedding degree should be 3,4 or 6" << endl;
        exit(0);
    }
...
// D must be square-free
                ...
                for (j=0;j<solutions;j++)
                {
                    ...
                    forever
                    {
					    ...
					    pFile << N << endl;
					    pFile << d << endl;
					    pFile << p << endl; pFile << nrp << endl; pFile << ord << endl;
                                        }
                                    }
                                //    else
